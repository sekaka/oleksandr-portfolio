...
export default function EditTimelineEntryPage({ params }: { params: { id: string } }) {
  ...
  const fetchEntry = async () => {
    try {
      ...
      if (response.ok) {
        ...
        alert('Timeline entry not found');
        router.push('/admin/timeline');
      }
    } catch (error) {
      console.error('Error fetching timeline entry:', error);
      alert('Failed to fetch timeline entry');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Failed to update timeline entry');
      }
    } catch (error) {
    ...
    <AuthGuard>
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Update timeline entry details</p>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-4">
                <Button type="submit" disabled={loading} className="admin-primary-btn">
                  {loading ? 'Updating...' : 'Update Experience Entry'}
                </Button>
